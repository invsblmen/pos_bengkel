...
export default function Show({ sale }) {
    ...
    return (
        <>
            <Head title={`Detail Penjualan ${sale?.invoice || ''}`} />
            <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Invoice: {sale?.invoice}</p>
                    </div>
                    <Link href={route('parts.sales.index')} className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-primary-600">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="bg-white rounded-2xl border border-slate-200 p-4">
                        ...
                        <div className="flex justify-between text-sm mb-2">
                            ...
                            <span className="font-medium">{sale?.created_at ? new Date(sale.created_at).toLocaleString('id-ID') : '-'}</span>
                        </div>
                        <div className="flex justify-between text-sm mb-2">
                        ...
                        <div className="flex justify-between text-sm mb-2">
                            ...
                            <span className="font-medium text-right">{sale?.notes || '-'}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                            <span className="text-slate-500">Total</span>
                            <span className="font-semibold text-primary-600">{totalFormatter.format(sale?.total || 0)}</span>
                        </div>
                    </div>
                </div>
